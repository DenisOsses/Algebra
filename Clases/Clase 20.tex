...
% Document metadata
\title{Clase 20}
\subtitle{\small{Recta vectorial. Producto Punto y ortogonalidad. Posiciones relativas de rectas}}
\author{Profesor: Denis Osses}
%\date{\AdvanceDate[+5]\today}
% \date{\today}
\date{28 y 30 de mayo de 2025}
...
\begin{center}
    \visible<2>{\includegraphics[scale=0.18]{EcVecRecta}}
\end{center}
...
\vspace{-0.5cm}
\begin{center}
    \includegraphics[scale=0.5]{AnguloVect1.png}
\end{center}\pause
Con la noción de ángulo entre vectores, podemos decidir cuándo ellos son \textbf{perpendiculares} u \textbf{ortogonales}, lo que ocurre cuando $\theta=\frac{\pi}{2}$. De este modo \pause
\begin{block}{Teorema}
\justifying
...
\begin{enumerate}[{(1)}]
\justifying
\item Suponga que $\vec{a}$ y $\vec{b}$ son dos vectores tales que $||\vec{a}||=2$, $||\vec{b}||=3$ y el ángulo que forman es de $\pi$ radianes. Calcule el valor de $(2\vec{a}-\vec{b})\cdot (\vec{a}-3\vec{b})$.
\item  Determine todos los valores de $k \in \mathbb{R}$ de modo que $\vec{a}=(-2k,k,1)$ sea perpendicular a $\vec{b}=(k,k,1)$.
\item Sean $\vec{a}$ y $\vec{b}$ vectores del espacio. Determine el ángulo entre $\vec{a}$ y $\vec{b}$, si sabe que $||\vec{a}-\vec{b}||=||\vec{a}+\vec{b}||$
\end{enumerate}
\end{exampleblock}
\end{minipage}
...
%---------------------------------------------------------------------
\subsection{Posiciones relativas de rectas}
\begin{frame}
  \frametitle{\insertsectionhead}
  \framesubtitle{\insertsubsectionhead}
...
\begin{block}{Definición}
\justifying
Dos rectas en el espacio (o en el plano) son \textbf{paralelas} si y solo si sus vectores directores son paralelos y dichas rectas no se cortan. \pause Luego, rectas coincidentes no son paralelas.
\end{block}\pause
