...
Si $\vec{a}=(a_1,a_2,a_3)$ y $\vec{b}=(b_1,b_2,b_3)$ entonces el \textbf{producto cruz} de $\vec{a}$ y $\vec{b}$ es el vector  $$\color{red}\boxed{\color{black}\vec{a}\times\vec{b}=(a_2b_3-a_3b_2,a_3b_1-a_1b_3,a_1b_2-a_2b_1)}$$
:::
   
\begin{minipage}[t]{0.5\linewidth}
A fin de hacer la definición más fácil de recordar, se usa la notación de determinantes.  Un \textbf{determinante de orden 2} se define mediante $$\begin{vmatrix}a&b\\c&d\end{vmatrix}=ad-bc.$$

Un \textbf{determinante de orden 3} se puede definir en términos de determinantes de segundo orden como sigue:  $$\begin{vmatrix}a&b&c\\a_1&a_2&a_3\\ b_1&b_2&b_3\end{vmatrix}=a\begin{vmatrix}a_2&a_3\\b_2&b_3\end{vmatrix}-b\begin{vmatrix}a_1&a_3\\b_1&b_3\end{vmatrix}+c\begin{vmatrix}a_1&a_2\\b_1&b_2\end{vmatrix}.$$
...
\begin{minipage}[t]{0.5\linewidth}
Ahora, si definimos los vectores (\textbf{canónicos}) del espacio: $\hat{i}=(1,0,0)$, $\hat{j}=(0,1,0)$ y $\hat{k}=(0,0,1)$,  podemos reescribir el producto cruz de $\vec{a}$ con $\vec{b}$ como  
