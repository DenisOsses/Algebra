...
Si $z=a+bi$, entonces su **forma polar** es $$z=r(\cos(\theta)+i\sen(\theta))=r\textrm{cis}(\theta)$$  donde $r=|z|=\sqrt{a^2+b^2}$ y $\tan(\theta)=\frac{b}{a}$.  El ángulo $\theta$ se llama **argumento** de $z$ y escribimos $\theta=\arg(z)$.  Note que $\arg(z)$ no es único.
```
...
$2\pi/n$, estas se encuentran igualmente espaciadas en este círculo.  Ver \href{https://www.geogebra.org/m/bterebwm}{**Geogebra**}
``` 
